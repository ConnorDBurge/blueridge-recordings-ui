...
export default function FlyoutMenu({ menu }) {
  ...
  return (
    <div className="absolute max-h-0 group-hover/menu-li:max-h-[75vh] overflow-hidden transition-300 rounded-b-lg ml-[-10px]">
              ...
              <Accordion header={subItem?.title}>
                {subItem?.items.map((accordionItem) => (
                  <Link
                    ...
                    className="no-underline text-gray-200 opacity-75 text-sm pr-1 transition-300 hover:text-colors-secondary hover:translate-x-1 hover:opacity-100"
                  >
                    {accordionItem?.title}
                  </Link>
                ))}
